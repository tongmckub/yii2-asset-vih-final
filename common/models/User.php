...
class User extends ActiveRecord implements IdentityInterface {
    ...
    const LOCATION_VIH = 10;
    const LOCATION_VIO = 20;
    const LOCATION_VIN = 30;
    const LOCATION_VIS = 40;
    ...
    public function rules() {
        return [
            ...
            ['location', 'in', 'range' => [self::LOCATION_VIH, self::LOCATION_VIO, self::LOCATION_VIN, self::LOCATION_VIS]],
    ...
    public function getSoftwares() {
        return $this->hasMany(Software::className(), ['created_by' => 'id']);
    }
    ...
    public function getSummaryOnSites() {
        return $this->hasMany(SummaryOnSite::className(), ['created_by' => 'id']);
    }

    /**
     * @return \yii\db\ActiveQuery
     */
    public function getSummaryOnSites0() {
        return $this->hasMany(SummaryOnSite::className(), ['updated_by' => 'id']);
    }
    ...
    public static function getItemsLocation() {
        return[
            self::LOCATION_VIH => 'ทั้งกลุ่ม',
            self::LOCATION_VIO => 'สาขาอ้อมน้อย',
            self::LOCATION_VIN => 'สาขาหนองแขม',
            self::LOCATION_VIS => 'สาขาสมุทรสาคร'
        ];
    }

    public function getLocationName() {
        $items = $this->getItemsLocation();
        return array_key_exists($this->location, $items) ? $items[$this->location] : '';
    }
